...
All trajectories are displayed in the Figure~\ref{fig:all_tracks_selected}. The trajectories of each group of the Table~\ref{tab:selected_experiments} can be found at Appendix~\ref{apendiceD}.

\newpage

\begin{figure}[!ht]
    \centering
    \caption{Hurricane Beryl track forecasts, ERA5, and best track dataset.} % Título acima da figura
...
The next figure provides complementary information to the analysis, now in a more quantitative manner. It is important to mention that the time series displays the errors after 12 h of spin-up, and that value is maintained for further analysis. In Figure \ref{fig:trackerrors} (a), it is possible to better distinguish the differences among the experiments over time. In other words, it shows when each experiment presents lower errors throughout the integration period, along with an initial comparison among them. It is important to note that the vertical line E indicates the moment the hurricane makes landfall on the Yucatán Peninsula, while the vertical line F marks its entry into the State of Texas. In Figure \ref{fig:trackerrors} (b), this comparison becomes even clearer, especially because the bars are arranged vertically from the lowest to the highest error (from top to bottom).

\newpage

\begin{figure}[!ht]
	\centering
	\caption{Track errors of the MONAN forecast for Hurricane Beryl and ERA5 reanalysis with the best track as reference. 12 hours of model spin-up are withdrawn from the analysis. (a) is displayed the distance error computed as the great-circle, and (b), the MAE and RMSE calculated from (a).} % Título acima da figura
...
Another feature that can be directly observed is that CP-15km is outperformed by CP-60km in this metric. This occurs because, with a coarser resolution, the probability of correctly identifying the location increases, as the number of grid cells is smaller compared to the high-resolution configuration. Consequently, the evaluation becomes less strict. Readers should keep this in mind, particularly for metrics in which the number of grid cells plays a decisive role.

Overall, forecasts tend to show errors below 100 km up to 42 hours of lead time, except for those initialized earlier (CP-29 and CP-02T12). Note that cold pools begin to influence after 48h of forecast. After 60 hours of forecast (two and a half days), some experiments begin to exceed 200 km of error. However, the cold pool only reaches this threshold after 84 hours (three and a half days), and the CP-D025 experiment only reaches it after about 111 hours (four and a half days). Therefore, we observe that with cold pool parameterization, forecast skill is extended by up to two days compared to the configuration without this scheme, for this specific case study.

In the literature, the best forecast limit reported is around 120 hours (five days) \cite{zhou2020prospects, sippel2022impacts}. It is worth emphasizing that forecast errors beyond this threshold should be considered “unpredictable” given the nonlinear nature of the system.
...
By observing Figure \ref{fig:all_tracks_selected} and Figure \ref{fig:trackerrors}, one could notice the westward deviation in the forecast. The next panel, showing the CTE and ATE, can confirm this behaviour, at Figure \ref{fig:cte}.

\begin{figure}[!ht]
	\centering
	\caption{CTE (a) and ATE (b) computing during the Hurricane Beryl period.} % Título acima da figura
...
The report  in the UK Met Office \cite{metoffice2024} ndicates that the track was forecasted with a notable degree of accuracy throughout the Caribbean region; furthermore, it was observed that there was a left-of-track bias in the forecasts as Beryl progressed into the Gulf of Mexico.
...
The mean error performance of all MONAN forecasts was computed and shown in Figure \ref{fig:monancomparedera5}.

\begin{figure}[!ht]
	\centering
	\caption{Overall performance of MONAN trajectory forecasts for Hurricane Beryl compared with ERA5 reanalysis.} % Título acima da figura
	\includegraphics[width=\textwidth]{docs/figuras/chapter5/barplot_monan_vs_era5_mae_rmse_FINAL.png} 
	\vspace{0.5em}
	Source: Made by the author.  % Fonte abaixo da imagem
	\label{fig:monancomparedera5} % Label para referenciar no texto
\end{figure}

In general, the MONAN forecast errors are approximately 8 times greater than the ones from the ERA5 reanalysis. Again, we can infer that it is not a fair comparison between the MONAN forecast and the ERA5 reanalysis, but it is important to notice the error scale to give us insights about improving the MONAN forecast.
