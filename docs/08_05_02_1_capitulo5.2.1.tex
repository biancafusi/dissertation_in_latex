...
At first glance, the reader will notice the observed congruence between the ERA5 reanalysis data and the best track observations. This strong agreement may be attributable to the reanalysis nature \citeonline{dulac2024assessing}, which integrates observational datasets throughout the integration period, thereby leading to this expected behaviour.
...
The next figure provides complementary information to the analysis, now in a more quantitative manner. It is important to mention that the time series displays the errors after 12h of spin-up, and that value is maintained for further analysis. In Figure \ref{fig:trackerrors} (a), it is possible to better distinguish the differences among the experiments over time. In other words, it shows when each experiment presents lower errors throughout the integration period, along with an initial comparison among them. It is important to note that the vertical line E indicates the moment the hurricane makes landfall on the Yucatán Peninsula, while the vertical line F marks its entry into the State of Texas. In Figure \ref{fig:trackerrors} (b), this comparison becomes even clearer, especially because the bars are arranged vertically from the lowest to the highest error (from top to bottom).
...
This westward bias was also noted in the forecasts from both the HAFS (Figure \ref{fig:all_tracks_selected}) and the United Kingdom (UK) Met Office (Figure \ref{fig:metoff}) for Hurricane Beryl, on which displays the forecast made initiating from different days.
